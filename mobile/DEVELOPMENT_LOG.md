...
## 二、v3.0.0 更新详情 (2026-04-09) - Web端功能对齐

### 3.1 Web端 vs APP端功能差异对照（已补齐）

| 功能 | Web端 | APP端(v2.x) | APP端(v3.0) |
|------|-------|-------------|-------------|
| AI智能管家 | ✅ GuestRoom AI标签 | ❌ 占位符 | ✅ 聊天界面+快捷问题+模拟AI响应 |
| 客房送物 | ✅ GuestRoom送物标签 | ❌ 占位符 | ✅ 分类浏览+数量选择+提交配送 |
| 联系前台 | ✅ GuestRoom前台标签 | ❌ 占位符 | ✅ 服务热线+语音通话+在线消息 |
| 设备控制 | ✅ GuestRoom(通过AI) | ✅ 独立页 | ✅ 4Tab整合 |
| 语音通话(前台) | ✅ VoiceCalls页 | ❌ 无 | ✅ 通话记录+快速呼叫+在线状态 |
| 房价设置(前台) | ✅ PriceSettings页 | ❌ 无 | ✅ 房型列表+价格编辑 |
| 工单处理(前台) | ✅ WorkOrders(含清洁) | ✅ 简单版 | ✅ 保留+9Tab整合 |
| 铃铛通知(前台) | ✅ 侧边栏红点 | ❌ 无 | ✅ 铃铛+通知面板+跳转 |
| 滑动Tab(前台) | ✅ 侧边栏9项 | ❌ 底部7项 | ✅ 顶部滑动9Tab |

### 3.2 顾客端客房服务重构 (room_service_page.dart)

从单页面重构为4Tab页面（对照Web端GuestRoom.vue）：

1. **AI智能管家** - 聊天界面，支持快捷问题标签，模拟AI响应（空调控制/送水/时间/天气/续住/WiFi/早餐/退房）
2. **客房送物** - 分类浏览（饮品/日用品/食品/其他），数量加减，备注，提交配送请求
3. **联系前台** - 服务热线（前台/客房服务/紧急电话），一键语音通话，在线消息面板
4. **设备控制** - IoT设备网格控制（灯光/空调/窗帘/电视/门锁），MQTT实时状态

### 3.3 前台端9Tab重构 (dashboard_page.dart)

从7个底部导航项重构为9个顶部滑动Tab（对照Web端ReceptionLayout）：

| Tab | 对应Web端 | 功能 |
|-----|----------|------|
| 总览 | Dashboard | 今日概况+工单+到店预订 |
| 入住退房 | CheckInOut | 待入住/待退房+办理 |
| 预订 | Bookings | 预订列表+状态筛选 |
| 客房 | RoomAvailability | 房间网格+状态修改 |
| 工单处理 | WorkOrders | 维修工单+处理 |
| 客房送物 | DeliveryOrders | 配送订单+状态跟踪 |
| 语音通话 | VoiceCalls | 通话记录+快速呼叫+在线状态 |
| 房价设置 | PriceSettings | 房型列表+价格编辑 |
| 账单报表 | Bills | 支付记录+详情弹窗 |

### 3.4 铃铛通知系统

- **通知铃铛**：AppBar右上角铃铛图标+红点数字
- **通知面板**：点击铃铛弹出底部面板，显示待处理工单和送物请求
- **跳转功能**：点击通知项直接跳转到对应Tab页

### 3.5 API字段对齐修复（续v2.1）

- `lookupBooking` 改用 `GET /bookings/lookup?keyword=`（修复在线入住查询不到预订）
- `getDashboardStats` 解析 `{ rooms: [{room_status, count}], bookings: [{status, count}] }` 格式
- `getRoomStatusDistribution` 从房间列表计算（后端无此端点）
- `getRooms` 使用 `room_status` 查询参数
- `updateRoom` 使用 `room_status` 字段更新

---

## 三、v2.1.0 更新详情 (2026-04-09)
