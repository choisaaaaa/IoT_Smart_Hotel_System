...
**管理端 · 前台端 · 客户端** 三端协同 | AI 客房管家 | IoT 设备实时监控 | 云边端混合架构
...
本项目采用创新的"**云端 - 边缘 - 设备端**"云边端一体化架构：

```
┌─────────────────────────────────────────────────────────┐
│  ☁️ 云端层（Cloud）：AI大脑 + 数据中台 + 管理平台         │
│  ├─ 智谱AI GLM-4：自然语言处理、智能决策                 │
│  ├─ 数据中台：住客画像、运营分析、预测预警               │
│  ├─ 管理平台：多酒店集团化管理、权限控制                 │
│  └─ 通信：MQTT Broker、WebSocket Server                 │
├─────────────────────────────────────────────────────────┤
│  🔷 边缘层（Edge）：本地智能 + 实时响应                   │
│  ├─ 主控：ESP32系列                                      │
│  ├─ 边缘计算：本地决策、离线自治、数据预处理              │
│  ├─ 实时控制：毫秒级设备响应、紧急报警                   │
│  └─ 通信：WiFi/MQTT、RS485、UART                        │
├─────────────────────────────────────────────────────────┤
│  🔌 设备层（Device）：感知 + 执行 + 交互                  │
│  ├─ 感知：温湿度、烟雾、雷达、门磁传感器                 │
│  ├─ 执行：灯光、空调、窗帘、电视、门锁控制器             │
│  └─ 交互：触摸屏、语音模块、RGB指示灯、SOS按键          │
└─────────────────────────────────────────────────────────┘
```
...
| **硬件** | v1.0.0 | 云边端一体化架构设计完成 |
| **更新日期** | 2026-04-16 | 品牌升级：智慧酒店物联网控制系统 → 慧宿智联 |

### 📋 更新日志

#### v2.2.0 (2026-04-16)
- **🎉 品牌升级**：
  - ✅ 项目名称升级为：慧宿智联·云边端一体化智能酒店物联网设备管理与服务全栈解决方案
  - ✅ 更新所有文档、代码、UI中的项目名称
  - ✅ 优化TypeScript类型定义，修复构建错误
...
- [云边端一体化硬件架构设计](./docs/02-硬件设计/云边端一体化硬件架构设计说明.md) - 详细硬件方案
