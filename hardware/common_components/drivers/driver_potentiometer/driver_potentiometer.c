...
static const char *TAG = "DRIVER_POT";
...
esp_err_t driver_potentiometer_init(int gpio_num)
{
    ...
    if (adc_oneshot_io_to_channel(gpio_num, &unit_id, &s_channel) != ESP_OK) {
        HAL_LOGE(TAG, "GPIO %d is not a valid ADC pin for this chip", gpio_num);
    ...
    s_inited = true;
    HAL_LOGI(TAG, "Potentiometer on GPIO %d (ADC unit %d)", gpio_num, (int)unit_id);
    return ESP_OK;
}
...
esp_err_t driver_potentiometer_read_percent(int *out_percent)
{
    ...
    int raw;
    esp_err_t err = driver_potentiometer_read_raw(&raw);
    if (err != ESP_OK) {
        return err;
    }
    if (raw < 0) {
