...
client.on('connect', () => {
    ...
    // 5. 周期性模拟后端下发新增楼控命令（占位业务流回归）
    setInterval(() => {
        const cmdTopic = `hotel/device/command/floor/${DEVICE_ID}`;
        const commands = ['broadcast_start', 'broadcast_stop', 'floor_reset'];
        const commandType = commands[Math.floor(Math.random() * commands.length)];
        const cmdPayload = {
            command_id: Math.floor(Date.now() / 1000),
            device_id: DEVICE_ID,
            command_type: commandType,
            created_by: "backend_simulator",
            timestamp: getTimestamp()
        };
        client.publish(cmdTopic, JSON.stringify(cmdPayload));
        console.log(`[Floor_03] 📤 模拟下发楼控命令: ${commandType}`);
    }, 65000);
});
