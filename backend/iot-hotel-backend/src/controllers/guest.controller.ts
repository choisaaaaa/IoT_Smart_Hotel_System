...
export const get = async (req: AuthRequest, res: Response) => {
  try {
    const { page = 1, pageSize = 10, booking_id, guest_name, guest_phone } = req.query;
    ...
    if (guest_name) {
      whereClause += ' AND g.guest_name LIKE ?';
      params.push(`%${guest_name}%`);
    }

    if (guest_phone) {
      whereClause += ' AND g.guest_phone = ?';
...
export const create = async (req: AuthRequest, res: Response) => {
  try {
    const { booking_id, guest_name, guest_phone, guest_id_number, check_in_time } = req.body;

    if (!booking_id || !guest_name || !guest_phone) {
      res.status(400).json(errorResponse('请求参数错误：缺少必要参数（booking_id, guest_name, guest_phone）'));
      return;
    }
    ...
    const [result] = await pool.query<ResultSetHeader>(
      'INSERT INTO guests (booking_id, guest_name, guest_phone, guest_id_number, check_in_time) VALUES (?, ?, ?, ?, ?)',
      [booking_id, guest_name, guest_phone, guest_id_number || null, check_in_time || new Date()]
    );
...
export const update = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { guest_name, guest_phone, guest_id_number, check_out_time } = req.body;
    ...
    if (guest_id_number !== undefined) {
      updateFields.push('guest_id_number = ?');
      params.push(guest_id_number);
    }
