...
async function testS01_MQTTAuth() {
  ...
  await test('mosquitto.conf 存在且配置正确', async () => {
    const fs = require('fs');
    const configPath = path.join(PROJECT_ROOT, 'config/mosquitto.conf');
  ...
  await test('mosquitto.passwd 密码文件存在', async () => {
    const fs = require('fs');
    const passwdPath = path.join(PROJECT_ROOT, 'config/mosquitto.passwd');
  ...
  await test('ACL访问控制文件存在', async () => {
    const fs = require('fs');
    const aclPath = path.join(PROJECT_ROOT, 'config/acl');
...
async function testS02_MQTTTLS() {
  ...
  await test('TLS证书生成脚本存在', async () => {
    const fs = require('fs');
    const scriptPath = path.join(PROJECT_ROOT, 'config/generate-tls-certs.bat');
...
async function testS04_MQTTTopicFormat() {
  ...
  await test('MQTT服务使用统一主题格式', async () => {
    const fs = require('fs');
    const path = require('path');
    const servicePath = path.join(PROJECT_ROOT, 'src/services/mqtt.service.ts');
...
async function testS05_SQLMerge() {
  ...
  await test('init.sql 包含完整的表结构', async () => {
    const fs = require('fs');
    const path = require('path');
    const sqlPath = path.join(PROJECT_ROOT, 'database/init.sql');
...
async function testH01_DeviceRegisterAuth() {
  ...
  await test('POST /devices/register 支持预注册Token认证', async () => {
    ...
    const fs = require('fs');
    const path = require('path');
    const routePath = path.join(PROJECT_ROOT, 'src/routes/v1/devices.ts');
...
async function testH02_WebSocketCORS() {
  ...
  await test('CORS配置生产环境限制', async () => {
    const fs = require('fs');
    const path = require('path');
    const appPath = path.join(PROJECT_ROOT, 'src/app.ts');
...
async function testH03_LoginLockout() {
  ...
  await test('LoginSecurityService 存在且可用', async () => {
    const fs = require('fs');
    const path = require('path');
    const servicePath = path.join(PROJECT_ROOT, 'src/services/login-security.service.ts');
    if (!fs.existsSync(servicePath)) {
...
async function testH04_DeviceKeyHash() {
  ...
  await test('设备密钥加密工具存在', async () => {
    const fs = require('fs');
    const path = require('path');
    const utilPath = path.join(PROJECT_ROOT, 'src/utils/device-key.ts');
  ...
  await test('设备密钥验证中间件存在', async () => {
    const fs = require('fs');
    const path = require('path');
    const authPath = path.join(PROJECT_ROOT, 'src/middleware/auth.ts');
...
async function testH05_AIKnowledgeBase() {
  ...
  await test('知识库服务使用正确的表名', async () => {
    const fs = require('fs');
    const path = require('path');
    const servicePath = path.join(PROJECT_ROOT, 'src/services/knowledge-base.service.ts');
    if (fs.existsSync(servicePath)) {
      const content = fs.readFileSync(servicePath, 'utf8');
      if (!content.includes('ai_knowledge_base')) {
...
async function testH06_VoiceGateway() {
  ...
  await test('VoiceGatewayService 存在', async () => {
    const fs = require('fs');
    const path = require('path');
    const servicePath = path.join(PROJECT_ROOT, 'src/services/voice-gateway.service.ts');
    if (!fs.existsSync(servicePath)) {
...
async function testH07_MQTTBrokerAddress() {
  ...
  await test('MQTT配置使用统一地址', async () => {
    const fs = require('fs');
    const path = require('path');
    const configPath = path.join(PROJECT_ROOT, 'src/config/mqtt.ts');
    if (!fs.existsSync(configPath)) {
...
async function testM04_PasswordPolicy() {
  ...
  await test('密码验证器要求8位+特殊字符', async () => {
    const fs = require('fs');
    const path = require('path');
    const validatorPath = path.join(PROJECT_ROOT, 'src/security/validators.ts');
...
async function testM02_ErrorHandling() {
  ...
  await test('error.ts 中间件区分生产/开发环境', async () => {
    const fs = require('fs');
    const path = require('path');
    const errorPath = path.join(PROJECT_ROOT, 'src/middleware/error.ts');
...
async function testM08_DockerMySQLPassword() {
  ...
  await test('docker-compose.yml 设置强默认密码', async () => {
    const fs = require('fs');
    const path = require('path');
    const composePath = path.join(PROJECT_ROOT, '../../docker/docker-compose.yml');
...
async function testL02_RequestId() {
  ...
  await test('request-id.ts 中间件文件存在', async () => {
    const fs = require('fs');
    const path = require('path');
    const middlewarePath = path.join(PROJECT_ROOT, 'src/middleware/request-id.ts');
...
async function testL03_DBConnectionPool() {
  ...
  await test('数据库连接池配置为30', async () => {
    const fs = require('fs');
    const path = require('path');
    const dbPath = path.join(PROJECT_ROOT, 'src/config/database.ts');
...
async function testArchitectureOptimizations() {
  ...
  await test('前端环境变量配置灵活化', async () => {
    const fs = require('fs');
    const path = require('path');
    const webPath = path.join(PROJECT_ROOT, '../../frontend/iot-hotel-web');
